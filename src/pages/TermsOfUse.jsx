...
export default function TermsOfUse() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const data = await ApiService.getPage('terms-of-use');
        setPageData(data);
      } catch (err) {
        console.error(err);
      ...
      <div className="legal-content-wrapper glass-panel">
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '30vh' }}>
            ...
            <p style={{ color: '#8c8f9c' }}>Loading terms of use...</p>
          </div>
        ) : error ? (
          ...
          <>
            <h1 className="legal-title">{pageData.title || 'Terms of Use & Service'}</h1>
